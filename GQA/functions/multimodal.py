...
class Multimodal:
    ...
    def plot_(self):
        x_ = arange(self.lbx,self.ubx,0.15)
        y_ = arange(self.lby,self.uby,0.15)
        ...
        ax = fig.add_subplot(1, 1,1, projection='3d')
        ax.plot_surface(X, Y, Z, rstride=1, cstride=1,linewidth=0, cmap = CMAP,antialiased=False, alpha = 0.3)
        plt.show()
    ...
    def plot_contour(self):
            ...
            CS = ax1.contourf(X,Y,Z,300,cmap = CMAP,origin=origin)
...
# m = Multimodal(lbx = -3, ubx = 12.1,lby = -4.1, uby = 5.9)
# m.plot_()
# m.plot_contour()